 ...
 *  - key_eq_check: A predicate function that checks for equivalency between
 * two keys.
...
static link_t *create_link (const elem_t key, const elem_t value,
                            link_t *next);
...
int
ioopm_create_hash_table (ioopm_hash_table_t **return_ht, size_t bucket_count,
                         ioopm_hash_function *hash_func,
                         ioopm_eq_function *key_eq_check,
                         ioopm_eq_function *value_eq_check)
{
  ...
  *return_ht = h_alloc_struct (global_heap, "ll*lll");
  ...
  // Assume bucket_count < 1 000 000 000
  char format_string[10];
  sprintf (format_string, "%ld*", bucket_count);
  bucket_t *bucket_array = h_alloc_struct (global_heap, format_string);
...
int
ioopm_hash_table_insert (ioopm_hash_table_t *ht, const elem_t key,
                         const elem_t value)
{
  // Check presumptions
  // Checks if the argument pointer is valid, returns an error status
  ...
  if (can_resize && ((ht)->key_count / (double)(ht)->bucket_count) > 1.5)
    {
...
int
ioopm_hash_table_lookup (bool *return_found, elem_t *return_value,
...
int
ioopm_hash_table_keys (ioopm_list_t **return_key_list,
...
int
ioopm_hash_table_values (ioopm_list_t **return_value_list,
                         const ioopm_hash_table_t *ht)
{
  // Check presumptions
  // Checks if the argument pointers are valid, returns an error status
...
int
ioopm_hash_table_remove (ioopm_hash_table_t *ht, const elem_t key)
{
  // Check presumptions
  // Checks if the argument pointers are valid, returns an error status
  if (ht == NULL)
...
int
ioopm_hash_table_has_key (bool *return_found, const ioopm_hash_table_t *ht,
                          const elem_t key)
{
  // Check presumptions
  // Checks if the argument pointers are valid, returns an error status
...
int
ioopm_hash_table_has_value (bool *return_found, const ioopm_hash_table_t *ht,
                            const elem_t value)
{
  // Check presumptions
  // Checks if the argument pointers are valid, returns an error status
...
int
ioopm_hash_table_size (size_t *return_size, const ioopm_hash_table_t *ht)
{
  // Check presumptions
  // Checks if the argument pointers are valid, returns an error status
  if (ht == NULL)
...
int
ioopm_hash_table_is_empty (bool *return_empty, const ioopm_hash_table_t *ht)
{
  // Check presumptions
  // Checks if the argument pointers are valid, returns an error status
  if (ht == NULL)
...
int
ioopm_hash_table_all (bool *return_all, const ioopm_hash_table_t *ht,
...
int
ioopm_hash_table_any (bool *return_any, const ioopm_hash_table_t *ht,
                      ioopm_hash_table_predicate *pred, const void *arg)
{
  // Check presumptions
  // Checks if the argument pointers are valid, returns an error status
 ...
 */
static void
update_value (link_t *link_ptr, void *new_value)
{
 ...
 */
static void
update_key_count (ioopm_hash_table_t **ht, int add)
{
  (*ht)->key_count += add;
}
 ...
 */
static void
get_value (link_t *link_ptr, void *return_value)
{
 ...
 * @param iterate_func A function pointer that is applied to a link specified
 * by the key.
 ...
 */
static bool
apply_to_one (ioopm_hash_table_t *ht, elem_t key,
 ...
 */
static int
prepend_key (elem_t key, elem_t *value_unusued __attribute__ ((unused)),
             void *key_list)
{
  int status = SUCCESS;
 ...
 */
static int
prepend_value (elem_t key_unusued __attribute__ ((unused)), elem_t *value,
               void *value_list)
{
  int status = SUCCESS;
 ...
 */
static int
remove_key_from_table (elem_t key,
                       elem_t *value_unusued __attribute__ ((unused)),
                       void *ht)
{
  int status = SUCCESS;
 ...
 */
static void
do_nothing (link_t *link_ignored __attribute__ ((unused)),
            void *args_ignored __attribute__ ((unused)))
{
  // Do nothing
}
...
/**
 * @brief Gets a pointer to a linked list head that matches to a given keys
 * hash function.
 ...
 */
static link_t *
get_linked_list_head (const ioopm_hash_table_t *ht, const elem_t key)
{
 ...
 */
static link_t *
create_link (const elem_t key, const elem_t value, link_t *next)
{
  link_t *new_link = h_alloc_struct (global_heap, "***");
  *new_link = (link_t){ .key_value_pair.key = key,
 ...
 */
static size_t
get_hash (const ioopm_hash_table_t *ht, const elem_t key)
{
  if (ht->hash_func == NULL)
    {
      // treat keys as integers
      return elem_to_size (
          key); // .integer reads the integer part of the elem_t
    }
  else
    {
 ...
 */
static size_t
get_index (const ioopm_hash_table_t *ht, const elem_t key)
{
...
get_key_value_pair (link_t *link_ptr)
{
 ...
 */
static link_t **
find_previous_pointer (const ioopm_hash_table_t *ht, link_t **first_link,
                       const elem_t key)
{
 ...
 * @param value_eq_check Function used when comapring values against each
 * other.
 */
static void
initialize_hash_table (ioopm_hash_table_t **ht, bucket_t *bucket_array,
                       size_t bucket_count, ioopm_hash_function *hash_func,
                       ioopm_eq_function *key_eq_check,
                       ioopm_eq_function *value_eq_check)
{
...
void
hash_table_resize (ioopm_hash_table_t **ht, ioopm_hash_function *hash_func,
                   ioopm_eq_function *key_eq_check,
                   ioopm_eq_function *value_eq_check)
{
  ...
  if (new_table->bucket_count < (*ht)->bucket_count)
    {
      can_resize = false;
      return;
    }
